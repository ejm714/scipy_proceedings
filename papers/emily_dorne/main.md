...
While complex models like CNNs often work well for imagery, simpler tree-based models have an advantage for point-based prediction. Tree-based features are determined and generated manually, therefore can more effectively focus on the area around the point of prediction. CNNs can uncover complex large-scale image features, but may not as effectively focus on a single point.
...
:width: 700px
Performance on the test set by region. There were 958 test data points in the midwest, 587 in the northeast, 740 in the south, and 595 in the west. Performance also varies notably by individual data provider[^footnote-5].
:::

Model accuracy can vary based on bloom severity as well as location and other attributes of the sampling point, so the performance metrics above will vary based on the distribution in the evaluation set. After concluding the model experimentation phase, we conducted a small out-of-sample evaluation using new data collected by California during summer 2023 (231 total observations). We found that estimated cyanobacteria densities consistently increased with the severity of the advisory level that was issued. While the relative ordering of points based on estimated severity was promising, absolute cyanobacteria densities were consistently overestimated. This reinforces that the main immediate use case of CyFi is to identify comparatively higher and lower priority areas, and to inform rather than replace ground sampling activities.
